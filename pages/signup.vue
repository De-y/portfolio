<template>
    <h1>STOP!</h1>
    <p>The signup route is not yet implemented. Please check back later.</p>
</template>
<!-- <script setup lang="tsx">
    ...
    const error = ref('');
    const success = ref('');
    async function sendLoginCredentials(username: string, password: string,email: string) {
        const loginCreds = await useFetch('/api/signup', {
            method: 'POST',
            headers: {
...
<template>
    ...
    <div class="arti">
        <a href="/" class="x">Portfolio</a> <br>
        <p>Log into the Portfolio with your username and password.</p><br>
        ...
        <button class="btn" @click="sendLoginCredentials(username, password,email)">Login</button><br>
        <div class="r">
